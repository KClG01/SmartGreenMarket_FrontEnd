...
/**
 * Trích flat array sản phẩm từ response lồng 2 tầng:
 * response.results[].results[]
 */
function extractProductList(response) {
  if (!response) return [];
  // Trường hợp response là array phẳng
  if (Array.isArray(response)) return response;
  // Trường hợp response.results là array sản phẩm phẳng (1 tầng)
  if (Array.isArray(response.results)) {
    const first = response.results[0];
    // Lồng 2 tầng: results[0].results là array sản phẩm
...
export default function CreateProductModal({ isOpen, onClose, onSuccess, mode = "catalog" }) {
  ...
  // product masters (đã lọc bỏ sản phẩm đang bán)
  ...
  // Cache set các product_master id đang active/approved của supplier
  ...
  // ── Fetch danh sách product_master id đang bán (active/approved) ──
  useEffect(() => {
    if (!isOpen) return;
    let cancelled = false;
    ...
    productService
      ...
      .then((response) => {
        ...
        const SELLING_STATUSES = new Set(["active", "approved"]);
        const sellingIds = new Set(
          allProducts
            .filter((p) => SELLING_STATUSES.has(p.status))
            .map((p) => {
              // product_master có thể là object hoặc id trực tiếp
              return p.product_master?.id ?? p.product_master ?? null;
            })
            .filter(Boolean)
            .map(String)
        );
        sellingMasterIdsRef.current = sellingIds;
      })
      .catch(() => {
        if (!cancelled) sellingMasterIdsRef.current = new Set();
      })
  ...
  // ── Fetch product masters theo category, lọc bỏ sản phẩm đang bán ──
  useEffect(() => {
    ...
    productMasterService
      ...
      .then((list) => {
        ...
        // Lọc bỏ các product master mà supplier đang bán rồi
        ...
        if (!isPersonal) {
          if (available.length > 0) {
            const stillExists = available.some((p) => String(p.id) === selectedProductId);
            if (stillExists) {
              const picked = available.find((p) => String(p.id) === selectedProductId);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        unit: form.unit,                          // ← fix: luôn gửi unit
      ...
      console.error("[CreateProductModal] response data:", err?.response?.status, err?.response?.data);
